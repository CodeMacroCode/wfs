...
export function EmployeeForm({
  ...
  const employeeObjId = useWatch({ control: form.control, name: "employeeObjId" });

  // Auto-assign or generate Employee ID
  useEffect(() => {
    // Only proceed if it's a new employee registration and we have a company selected
    if (isEdit || !companyId || !prefix || isGeneratingId) return;

    // If we have available IDs and none is selected, auto-pick the first one
    if (employeeIdData?.data && employeeIdData.data.length > 0) {
      if (!employeeObjId) {
        form.setValue("employeeObjId", employeeIdData.data[0]._id, { shouldValidate: true });
      }
    } 
    // If no IDs are available for the company, none is selected, and we haven't already tried generating for this company
    else if (employeeIdData && employeeIdData.data.length === 0 && !employeeObjId && hasAutoGeneratedFor.current !== companyId) {
      hasAutoGeneratedFor.current = companyId;
      generateEmployeeId({ prefix, remark: "Auto-generated during registration" });
    }
  }, [employeeIdData, companyId, prefix, form, isEdit, isGeneratingId, generateEmployeeId, employeeObjId]);
