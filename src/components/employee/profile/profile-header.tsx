...
export function ProfileHeader({ employee }: ProfileHeaderProps) {
  const previewUrl = typeof employee.profilePicture === 'string' ? employee.profilePicture : null;

  return (
    <div className="relative mb-6">
      {/* Banner */}
      <div className="h-48 w-full rounded-t-2xl bg-gradient-to-r from-emerald-400 via-teal-500 to-cyan-600 relative overflow-hidden shadow-sm">
      ...
      <div className="px-8 pb-4">
        <div className="flex flex-col md:flex-row items-end gap-6 -mt-12 mb-4 relative z-10">
          ...
          <div className="w-32 h-32 rounded-full border-4 border-white shadow-xl overflow-hidden bg-white shrink-0">
            {previewUrl ? (
              <img src={previewUrl} alt={employee.name} className="w-full h-full object-cover" />
            ) : (
